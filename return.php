      ...
      <table class='table table-bordered align-middle' id='returnTable'>
        ...
        <tbody>
          <tr class='return-row'>
            ...
            <td><input class='form-control pic-input' name='pic[]' placeholder='Returned by'></td>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  function refreshRemoveButtons(){ const rows = tbody.querySelectorAll('tr.return-row'); rows.forEach(row => row.querySelector('.remove-row').disabled = rows.length === 1); }
  function clearRow(row){
    row.querySelector('.serial-input').value=''; row.querySelector('.qty-input').value=1; row.querySelector('.pic-input').value='';
    row.querySelector('.found-item').textContent='Waiting for scan...'; row.querySelector('.found-item').className='found-item text-muted'; row.querySelector('.issued-text').textContent='-';
  }
  function lookup(row){
    const serial=row.querySelector('.serial-input').value.trim(); const found=row.querySelector('.found-item'); const issued=row.querySelector('.issued-text');
    if(!serial){ found.textContent='Waiting for scan...'; issued.textContent='-'; return; }
    fetch('ajax/get_item.php?serial='+encodeURIComponent(serial)).then(r=>r.json()).then(data=>{
      if(data.ok){ found.textContent=data.item.item_description+' | '+(data.item.location||'No location'); found.className='found-item text-success fw-semibold'; issued.textContent=data.item.outstanding_issued ?? 0; }
      else { found.textContent='Item/code not found'; found.className='found-item text-danger fw-semibold'; issued.textContent='0'; }
    }).catch(()=>{ found.textContent='Lookup failed'; found.className='found-item text-danger'; });
  }
  addBtn.addEventListener('click', function(){ const last=tbody.querySelector('tr.return-row:last-child'); const row=last.cloneNode(true); clearRow(row); tbody.appendChild(row); refreshRemoveButtons(); row.querySelector('.serial-input').focus(); });
  tbody.addEventListener('click', function(e){ if(e.target.classList.contains('remove-row')){ e.target.closest('tr').remove(); refreshRemoveButtons(); } });
  tbody.addEventListener('blur', function(e){ if(e.target.classList.contains('serial-input')) lookup(e.target.closest('tr')); }, true);
  tbody.addEventListener('keydown', function(e){ if(e.key==='Enter' && e.target.classList.contains('serial-input')){ e.preventDefault(); lookup(e.target.closest('tr')); e.target.closest('tr').querySelector('.qty-input').focus(); } });
});
</script>
