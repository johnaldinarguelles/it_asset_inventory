...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $loc = trim($_POST['location'] ?? 'Storage Room');
  ...
  if ($desc === '') $err = 'Item description is required.';
    ...
    $n = 2;
    while (true) {
      $st = $conn->prepare('SELECT id FROM no_serial_items WHERE item_code=? UNION SELECT id FROM items WHERE serial_number=? LIMIT 1');
      $st->bind_param('ss', $code, $code);
      $st->execute();
      if (!$st->get_result()->fetch_assoc()) break;
      $code = $base . '-' . $n++;
    }
    $conn->begin_transaction();
    try {
      $st = $conn->prepare('INSERT INTO no_serial_items(item_description,item_code,default_location,uom,created_by) VALUES(?,?,?,?,?)');
      $st->bind_param('ssssi', $desc, $code, $loc, $uom, $_SESSION['user_id']);
      $st->execute();
      ...
      $st->bind_param('ssss', $desc, $code, $loc, $uom);
      $st->execute();
      $conn->commit();
      $msg = 'No-serial item added. Use item code ' . $code . ' for receiving, issuance, and return.';
    } catch (Throwable $e) {
      $conn->rollback();
      $err = 'Cannot add item. It may already exist.';
    }
  }
}
...
<?php if ($err): ?><div class='alert alert-danger d-flex align-items-center gap-2'><i class='bi bi-exclamation-triangle-fill'></i><span><?= e($err) ?></span></div><?php endif; ?>
<div class='alert alert-info d-flex align-items-center gap-2'><i class='bi bi-info-circle-fill'></i><span>For items without unique serial numbers, this page creates one <b>general item code</b>. Example: all AA Battery transactions use the same generated code. Editing is disabled to protect report accuracy; add a new item only when needed.</span></div>
<div class='row g-3'>
  <div class='col-lg-4'>
    <div class='card cardx p-3'>
      <h5 class='mb-3'><i class='bi bi-plus-circle'></i> Add No-Serial Item</h5>
      <form method='post'>
  ...
  <div class='col-lg-8'>
    <div class='card cardx p-3'>
      <h5 class='mb-3'><i class='bi bi-list-ul'></i> Registered No-Serial Items</h5>
      <table class='table table-hover datatable'>
        <thead>
          <tr>
            <th>ID</th>
            <th>Description</th>
            <th>General Item Code</th>
            <th>Location</th>
            <th>UOM</th>
            <th>Received</th>
            <th>Usage</th>
        ...
        <tbody>
          <?php while ($r = $res->fetch_assoc()): ?><tr>
              ...
              <td><code><?= e($r['item_code']) ?></code></td>
              <td><?= e($r['default_location']) ?></td>
              <td><?= e($r['uom']) ?></td>
