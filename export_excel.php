...
if($type==='transactions' || $type==='report'){
    ...
    if($type==='report'){
        echo '<tr><th colspan="8">Inventory Movement Summary</th></tr>';
        echo '<tr><th>Item</th><th>Serial / Code</th><th>Location</th><th>Total Received</th><th>Total Usage / Issued</th><th>Total Return</th><th>Total Disposed</th><th>Net Movement</th></tr>';
        ...
        while($r=$res->fetch_assoc()) echo '<tr><td>'.e($r['item_description']).'</td><td>'.e($r['serial_number']).'</td><td>'.e($r['location']).'</td><td>'.$r['total_received'].'</td><td>'.$r['total_issued'].'</td><td>'.$r['total_returned'].'</td><td>'.$r['total_disposed'].'</td><td>'.$r['net_movement'].'</td></tr>';
        echo '<tr></tr><tr><th colspan="8">Detailed Transaction Report</th></tr>';
    }
    echo '<tr><th>ID</th><th>Date When</th><th>What / Action</th><th>Serial / Code</th><th>Item</th><th>Qty</th><th>PIC</th><th>Week</th><th>Location</th><th>Remarks</th></tr>';
    ...
    while($r=$res->fetch_assoc()) echo '<tr><td>'.$r['id'].'</td><td>'.$r['created_at'].'</td><td>'.$r['action_type'].'</td><td>'.e($r['serial_number']).'</td><td>'.e($r['item_description']).'</td><td>'.$r['quantity'].'</td><td>'.e($r['pic']).'</td><td>'.$r['week_no'].'</td><td>'.e($r['location']).'</td><td>'.e($r['remarks']).'</td></tr>';
}
elseif($type==='monthly'){
    ...
    echo '<tr><th>ID</th><th>Description</th><th>Serial / Code</th><th>Location</th><th>UOM</th><th>BOH</th><th>Total Received</th><th>Total Usage / Issued</th><th>Total Returned</th><th>Total Disposed</th><th>Total Stock</th><th>Actual</th><th>Variance</th><th>Status</th></tr>'; $res=$conn->query('SELECT *,(boh+total_received+total_returned-total_issued-total_disposed) stock,(actual_stock-(boh+total_received+total_returned-total_issued-total_disposed)) variance FROM items'); while($r=$res->fetch_assoc()) echo '<tr><td>'.$r['id'].'</td><td>'.e($r['item_description']).'</td><td>'.e($r['serial_number']).'</td><td>'.e($r['location']).'</td><td>'.e($r['uom']).'</td><td>'.$r['boh'].'</td><td>'.$r['total_received'].'</td><td>'.$r['total_issued'].'</td><td>'.$r['total_returned'].'</td><td>'.$r['total_disposed'].'</td><td>'.$r['stock'].'</td><td>'.$r['actual_stock'].'</td><td>'.$r['variance'].'</td><td>'.$r['status'].'</td></tr>';
}
echo '</table>';
?>
