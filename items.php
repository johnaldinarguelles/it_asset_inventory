...
$res=$conn->query("SELECT *,(boh+total_received+total_returned-total_issued-total_disposed) stock, ((boh+total_received+total_returned-total_issued-total_disposed)-actual_stock) variance FROM items ORDER BY updated_at DESC"); ?>
<div class='page-head'>
  <div><h3><i class='bi bi-box-seam'></i> Items &amp; Real-time Stock</h3><p class='page-sub'>Use <b>View</b> to see all receiving, issuance, return, and disposal activity for an item. For non-unique items (mouse, office supplies), use one general barcode/item code &mdash; example: all mouse transactions use <b>5718185</b>. Stock = Received + Returned &minus; Issued &minus; Disposed.</p></div>
...
<div class='card cardx p-3'><table class='table table-hover datatable'><thead><tr><th>ID</th><th>Description</th><th>Serial</th><th>Classification</th><th>Location</th><th>UOM</th><th>Total Received</th><th>Total Usage</th><th>Total Return</th><th>Total Disposed</th><th>Total Stocks</th><th>Actual</th><th>Variance</th><th>Status</th><th>Action</th></tr></thead><tbody>
<?php while($r=$res->fetch_assoc()): ?><tr class='<?= $r['stock']<=0?'out':($r['stock']<=$r['reorder_level']?'low':'') ?>'><td><?=$r['id']?></td><td><?=e($r['item_description'])?></td><td><?=e($r['serial_number'])?></td><td><?=e($r['asset_classification'])?></td><td><?=e($r['location'])?></td><td><?=e($r['uom'])?></td><td><?=$r['total_received']?></td><td><?=$r['total_issued']?></td><td><?=$r['total_returned']?></td><td><?=$r['total_disposed']?></td><td><?=$r['stock']?></td><td><?=$r['actual_stock']?></td><td><?=$r['variance']?></td><td><span class='badge bg-secondary'><?=e($r['status'])?></span></td><td><button type='button' class='btn btn-sm btn-outline-info' onclick='openActivityModal(<?=$r['id']?>)' aria-label='View activity for <?=e($r['item_description'])?>'><i class='bi bi-eye'></i> View</button> <?php if(is_admin()): ?><button class='btn btn-sm btn-outline-primary' onclick='openItemModal(<?=$r['id']?>)' aria-label='Edit <?=e($r['item_description'])?>'><i class='bi bi-pencil'></i> Edit</button> <button class='btn btn-sm btn-outline-danger' onclick='deleteItem(<?=$r['id']?>)' aria-label='Delete <?=e($r['item_description'])?>'><i class='bi bi-trash'></i> Delete</button><?php endif; ?></td></tr><?php endwhile; ?></tbody></table></div>
