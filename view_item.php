...
$id=(int)($_GET['id']??0);
$st=$conn->prepare('SELECT *, (boh+total_received+total_returned-total_issued-total_disposed) stock, ((boh+total_received+total_returned-total_issued-total_disposed)-actual_stock) variance FROM items WHERE id=?');
...
?>
<div class='page-head'><div><h3><i class='bi bi-clock-history'></i> Item Activity View</h3><p class='page-sub'>Full receiving, issuance, return, and disposal history for this item</p></div><a href='items.php' class='btn btn-outline-secondary'><i class='bi bi-arrow-left'></i> Back</a></div>
<div class='row g-3 mb-3'>
 ...
 <div class='col-md-2'><div class='card cardx p-3'><small>Total Disposed</small><h4><?=$item['total_disposed']?></h4></div></div>
...
<div class='card cardx p-3 mb-3'><form class='row g-2'>
 ...
 <div class='col-md-3'><select class='form-select' name='action_type'><option value=''>All Activities</option><?php foreach(['Received','Issued','Returned','Disposed','Adjusted'] as $a):?><option value='<?=$a?>' <?=($_GET['action_type']??'')===$a?'selected':''?>><?=$a?></option><?php endforeach;?></select></div>
